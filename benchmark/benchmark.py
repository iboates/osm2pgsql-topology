...
def main():
    ...
    from_nodes_df = pd.read_sql("SELECT id as from_node FROM public.ways_vertices_pgr ORDER BY RANDOM() LIMIT 10;", engine)
    to_nodes_df = pd.read_sql("SELECT id as to_node FROM public.ways_vertices_pgr ORDER BY RANDOM() LIMIT 10;", engine)
    ...
    for i, node_pair in node_pairs_df.iterrows():
        ...
        gdf = gpd.read_postgis(sa.text(f"""
                    ...
                    where
                        gid in (select edge from pgr_res)
                """), engine, geom_col="the_geom")
        gdf.to_postgis("route_result", engine, schema="pgr", if_exists="append")
